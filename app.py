...
def stone_card(shape, carat, color, clarity, score, price, meta, tags):
    tag_html = "".join(f'<span class="tag {cls}">{label}</span>' for label, cls in tags)
    return f"""
    <div class="stone-card">
      <div class="stone-main">
        <div>{shape}</div><div>{carat}</div><div>{color}</div><div>{clarity}</div><div>{score}</div><div class="price">{price} ₽</div>
      </div>
      <div class="stone-line"></div>
      <div class="stone-meta"><div>{meta}</div><div class="tags">{tag_html}</div></div>
...
cards_html = "".join([
    stone_card("Круг", "1.05", "G", "VVS1", "95", "32 200", "6.5 мм · ex ex vg · none", [("огонь", ""), ("блеск", "tag-blue")]),
    stone_card("Круг", "1.51", "E", "VS1", "86", "58 500", "7.4 мм · ex ex vg · none", [("контраст", "tag-gray"), ("огонь", "")]),
    stone_card("Круг", "1", "F", "VS2", "77", "34 900", "6.4 мм · ex ex vg · none", [("БАЛАНС", "")]),
    stone_card("Круг", "1.22", "D", "VVS2", "92", "41 800", "6.9 мм · ex ex ex · none", [("блеск", "tag-blue"), ("огонь", "")]),
    stone_card("Круг", "1.74", "F", "VS1", "89", "67 900", "7.8 мм · ex ex vg · faint", [("контраст", "tag-gray"), ("блеск", "tag-blue")]),
    stone_card("Круг", "2.03", "G", "VS2", "84", "88 400", "8.1 мм · ex vg vg · none", [("БАЛАНС", "")]),
    stone_card("Круг", "2.41", "E", "VVS1", "96", "126 300", "8.7 мм · ex ex ex · none", [("огонь", ""), ("блеск", "tag-blue")]),
])
...
function openSheet(){ app.classList.add('sheet-open'); sheet.style.transition='transform .22s ease'; sheet.style.transform='translate(-50%, 0)'; }
function closeSheet(){ sheet.style.transition='transform .22s ease'; sheet.style.transform='translate(-50%, 110%)'; setTimeout(()=>app.classList.remove('sheet-open'),180); }
...
sheet.addEventListener('touchend', e => { if(!dragging) return; dragging=false; const touch=e.changedTouches[0]; const dy=touch.clientY-startY; const dx=Math.abs(touch.clientX-startX); if(dy>80 && dx<100 && sheet.scrollTop <= 5){ closeSheet(); } else { sheet.style.transition='transform .18s ease'; sheet.style.transform='translate(-50%, 0)'; } }, {passive:true});
</script>
</body>
</html>
"""
