...
if st.button("open_filters_hidden", key="open_filters_hidden", help="open", type="secondary"):
    st.query_params["page"] = "catalog"
    st.query_params["filters"] = "1"
    st.rerun()

if st.button("close_filters_hidden", key="close_filters_hidden", help="close", type="secondary"):
    st.query_params.clear()
    st.query_params["page"] = "catalog"
    st.rerun()

filters_open = st.query_params.get("filters") == "1"
...
div[data-testid="stButton"]:has(#open_filters_hidden),div[data-testid="stButton"]:has(#close_filters_hidden){display:none}.st-key-open_filters_hidden,.st-key-close_filters_hidden{display:none}
.catalog-header{display:flex;align-items:center;gap:1rem;padding:1.4rem 1rem 1.15rem;border-bottom:1px solid #ddd;box-shadow:0 2px 8px rgba(0,0,0,.12);background:#fff}.logo-title{font-family:serif;font-size:1.35rem;letter-spacing:.08em;color:#111}.logo-sub{margin-top:.35rem;color:#8b8b8b;font-size:.72rem;letter-spacing:.08em}.logo-mark{width:72px;height:44px;border:1.4px solid #111;border-radius:50%;position:relative;flex:0 0 auto}.logo-mark:after{content:"";position:absolute;width:72px;height:44px;border:1.4px solid #111;border-radius:50%;left:32px;top:-1px}.catalog-top{display:grid;grid-template-columns:2fr 1fr;gap:.65rem;padding:1.35rem 1rem .8rem}.catalog-select,.catalog-pick{border:1px solid #aaa;border-radius:14px;min-height:70px;display:flex;align-items:center;justify-content:center;background:#fff}.catalog-select{justify-content:space-between;padding:0 1rem}.select-title{font-weight:700;font-size:1.05rem;color:#111}.select-sub{color:#777;font-size:.92rem;margin-top:.25rem}.catalog-pick{color:#555;font-size:.98rem;line-height:1.35;text-align:center}.catalog-cols,.stone-main{display:grid;grid-template-columns:1fr .82fr .62fr .92fr .78fr 1.25fr;gap:.2rem}.catalog-cols{color:#aaa;font-size:.65rem;padding:.45rem 1.95rem .6rem}.stone-card{margin:0 1rem 1rem;border:1px solid #d0d0d0;border-radius:18px;padding:1.05rem 1rem .75rem;background:#fff}.stone-main{align-items:start;font-size:1rem;color:#111}.price{font-weight:700;text-align:right;font-size:1.03rem;line-height:1.25}.rub{font-size:.92rem}.stone-line{border-top:1px solid #e6e6e6;margin:1.05rem 0 .7rem}.stone-meta{display:flex;justify-content:space-between;align-items:center;color:#666;font-size:.86rem;gap:.5rem}.tags{display:flex;gap:.35rem;flex-wrap:wrap;justify-content:flex-end}.tag{border:1px solid #f0c56d;color:#c77c16;border-radius:7px;padding:.08rem .45rem;font-size:.7rem}.tag-blue{border-color:#8bd7e8;color:#1892ad}.tag-gray{border-color:#ccc;color:#555}.actions{display:grid;grid-template-columns:repeat(6,1fr);color:#888;font-size:1.25rem;padding-top:.85rem;text-align:center}.filter-sort{position:fixed;left:0;right:0;bottom:4.25rem;z-index:999998;background:#fff;border-top:1px solid #ddd;padding:.5rem .8rem}.filter-inner{max-width:430px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr;gap:.65rem}.filter-box{border:1px solid #c9c9c9;border-radius:14px;padding:.55rem .8rem;min-height:46px;background:#fff;text-decoration:none!important;display:block;color:#111!important}.filter-label{color:#aaa;font-size:.72rem;letter-spacing:.04em}.filter-value{font-weight:700;font-size:.92rem;color:#111;margin-top:.12rem}.filter-button-real{position:fixed;right:.8rem;bottom:4.25rem;width:calc(50% - 1.125rem);height:64px;z-index:1000001;opacity:0}.close-button-real{position:fixed;left:0;right:0;top:0;bottom:0;z-index:1000000;opacity:0}.filter-overlay{position:fixed;left:0;right:0;top:0;bottom:0;z-index:999999;background:rgba(0,0,0,.22)}.filter-sheet{position:fixed;left:0;right:0;bottom:0;z-index:1000002;max-width:430px;margin:0 auto;background:#fff;border:1px solid #999;border-bottom:0;border-radius:26px 26px 0 0;padding:1.4rem 1.35rem 1.2rem;box-shadow:0 -6px 22px rgba(0,0,0,.18)}.sheet-handle{width:42px;height:4px;border-radius:4px;background:#c9c9c9;margin:0 auto 1rem}.sheet-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:.7rem}.sheet-title{font-weight:700;font-size:1.15rem;color:#111}.reset{font-size:.78rem;color:#222;text-decoration:none!important}.filter-group{margin:.85rem 0 1.05rem}.filter-name{font-size:.76rem;font-weight:600;color:#111;margin-bottom:.55rem}.chips{display:flex;gap:.55rem;flex-wrap:wrap}.chip{border:1px solid #aaa;border-radius:18px;padding:.45rem .82rem;font-size:.67rem;color:#111;background:#fff}.chip-on{background:#000;color:#fff;border-color:#000}.chip-note{font-size:.58rem;color:#222;align-self:center}.kurgin-bottom-nav{position:fixed;left:0;right:0;bottom:0;width:100%;z-index:999999;background:#fff;border-top:1px solid rgba(49,51,63,.18);padding:.25rem .25rem calc(.45rem + env(safe-area-inset-bottom))}.kurgin-nav-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:.2rem;max-width:430px;margin:0 auto}.kurgin-tab{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:52px;border-radius:12px;color:#888!important;text-decoration:none!important;font-size:.68rem;line-height:1.1;white-space:nowrap}.kurgin-tab-active{background:#f2f5f8;color:#111!important;border:1px solid #cfd5dc}.kurgin-icon{font-size:1.15rem;line-height:1;margin-bottom:.15rem}.page-pad{padding:1rem}
</style>
...
<div class="filter-sort"><div class="filter-inner"><div class="filter-box"><div class="filter-label">СОРТИРОВКА</div><div class="filter-value">по Karo Score ↓</div></div><div class="filter-box"><div class="filter-label">ФИЛЬТРЫ</div><div class="filter-value">☷ Параметры</div></div></div></div>
""", unsafe_allow_html=True)
    st.button("", key="open_filters_button", on_click=lambda: st.query_params.update({"page":"catalog","filters":"1"}))
    st.markdown('<style>.st-key-open_filters_button{position:fixed;right:.8rem;bottom:4.25rem;width:calc(50% - 1.125rem);height:64px;z-index:1000001;opacity:0}</style>', unsafe_allow_html=True)
    if filters_open:
        st.markdown('<div class="filter-overlay"></div>', unsafe_allow_html=True)
        st.button("", key="close_filters_button", on_click=lambda: (st.query_params.clear(), st.query_params.update({"page":"catalog"})))
        st.markdown('<style>.st-key-close_filters_button{position:fixed;left:0;right:0;top:0;bottom:0;z-index:1000000;opacity:0}</style>', unsafe_allow_html=True)
        st.markdown("""
